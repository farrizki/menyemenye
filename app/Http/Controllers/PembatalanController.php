...
{
    // ... (SEMUA METHOD DARI create() SAMPAI store() TETAP SAMA, TIDAK DIUBAH) ...
    // ... Saya akan langsung ke method yang kita ubah dan tambahkan ...

    // METHOD LAMA (index, create, preview, store) TIDAK SAYA TULIS ULANG, BIARKAN SEPERTI ASLINYA
    ...
    /**
     * Method BARU: Menampilkan preview untuk proses EDIT.
     */
    public function previewUpdate(Request $request, Pembatalan $pembatalan)
    {
        $validatedData = $request->validate([
            ...
            'tahun_sk' => 'required|integer|min:2000',
            'tgl_sk' => 'required|date',
            'berkas' => 'nullable|file|mimes:pdf|max:24576', // Berkas tidak wajib saat edit
        ]);

        // Rekonstruksi No SK dan Keterangan
        $validatedData['no_sk'] = '100.3.3.2/' . $request->input('nomor_sk') . '/K/411.403/' . $request->input('tahun_sk');
        $validatedData['keterangan_pembatalan'] = 'batal sk ' . $validatedData['no_sk'];
        
        // Menangani file upload sementara, konsisten dengan controller lain
        if ($request->hasFile('berkas')) {
            $berkasPath = $request->file('berkas')->store('temp_pembatalan', 'local');
            $request->session()->put('berkas_temp_path_update', $berkasPath);
        } else {
             $request->session()->forget('berkas_temp_path_update');
        }

        // Kita kirim data ke view preview yang SAMA dengan proses create
        // Namun, kita gunakan variabel yang berbeda agar tidak bentrok
        return view('pembatalan.preview', [
            'isEdit' => true,
            'existingData' => $pembatalan, // Data lama dari database
            'editData' => $validatedData, // Data baru dari form edit
        ]);
    }
    
    /**
     * Mengupdate data pembatalan (hanya log dan berkas).
     * Disesuaikan untuk menerima data dari preview.
     */
    public function update(Request $request, Pembatalan $pembatalan)
    {
        // Validasi ini sebenarnya sudah dilakukan di previewUpdate,
        // tapi baik untuk keamanan jika ada yang mengakses URL langsung.
        $validatedData = $request->validate([
            'no_sk' => 'required|string|max:255',
            'tgl_sk' => 'required|date',
            'keterangan_pembatalan' => 'required|string',
        ]);

        // Handle upload berkas dari session (jika ada file baru)
        ...
        if ($berkasTempPath && Storage::disk('local')->exists($berkasTempPath)) {
            ...
            // Pindahkan file baru dari temp ke public
        ...
        $pembatalan->update([
            ...
            'keterangan_pembatalan' => $validatedData['keterangan_pembatalan'],
    ...
    public function destroy(Pembatalan $pembatalan)
    {
        ...
        try {
            DafnomOp::on('oracle')
                ->where('kd_propinsi', $pembatalan->kd_propinsi)->where('kd_dati2', $pembatalan->kd_dati2)
                ->where('kd_kecamatan', $pembatalan->kd_kecamatan)->where('kd_kelurahan', $pembatalan->kd_kelurahan)
                ->where('kd_blok', $pembatalan->kd_blok)->where('no_urut', $pembatalan->no_urut)->where('kd_jns_op', $pembatalan->kd_jns_op)
                ->where('thn_pembentukan', Carbon::parse($pembatalan->created_at)->year)
                ->update(['kategori_op' => 4, 'keterangan' => null, 'tgl_pemutakhiran' => null, 'nip_pemutakhir' => null]);
            ...
            if ($pembatalan->status_proses == 'Siap Diproses') {
                Sppt::on('oracle')
                    ...
                    ->where('kd_kecamatan', $pembatalan->kd_kecamatan)->where('kd_kelurahan', $pembatalan->kd_kelurahan)
                    ->where('kd_blok', $pembatalan->kd_blok)->where('no_urut', $pembatalan->no_urut)->where('kd_jns_op', $pembatalan->kd_jns_op)
                    ->where('thn_pajak_sppt', $pembatalan->thn_pajak_sppt)->where('status_pembayaran_sppt', 2)
    ...
    // ... (SISA SEMUA METHOD HELPER DAN CETAK BIARKAN SEPERTI ASLINYA) ...
    private function getPreviewData($nop, $tahun) {
        $sppt = Sppt::on('oracle')
            ->whereRaw("kd_propinsi||kd_dati2||kd_kecamatan||kd_kelurahan||kd_blok||no_urut||kd_jns_op = ?", [$nop])
            ->where('thn_pajak_sppt', $tahun)->first();
        if(!$sppt) return [];
        $op = DatObjekPajak::on('oracle')
            ->where('kd_propinsi', $sppt->kd_propinsi)->where('kd_dati2', $sppt->kd_dati2)->where('kd_kecamatan', $sppt->kd_kecamatan)
            ->where('kd_kelurahan', $sppt->kd_kelurahan)->where('kd_blok', $sppt->kd_blok)->where('no_urut', $sppt->no_urut)
            ->where('kd_jns_op', $sppt->kd_jns_op)->first();
        ...
        return [
            'nm_wp_sppt' => $sppt->nm_wp_sppt, 'alamat_wp' => $alamatWp, 'alamat_op' => $alamatOp,
            'luas_bumi' => $sppt->luas_bumi_sppt, 'luas_bangunan' => $sppt->luas_bng_sppt, 'pbb_baku' => $sppt->pbb_yg_harus_dibayar_sppt,
        ];
    }
    private function logPembatalan($item, $previewData, $user, $berkasPath) {
        $nop = $item['nop'];
        Pembatalan::create([
            'kd_propinsi' => substr($nop, 0, 2), 'kd_dati2' => substr($nop, 2, 2), 'kd_kecamatan' => substr($nop, 4, 3),
            'kd_kelurahan' => substr($nop, 7, 3), 'kd_blok' => substr($nop, 10, 3), 'no_urut' => substr($nop, 13, 4), 'kd_jns_op' => substr($nop, 17, 1),
            ...
            'pbb_yg_harus_dibayar_sppt' => $item['data_preview']['pbb_baku'] ?? 0, 'luas_bumi_sppt' => $item['data_preview']['luas_bumi'] ?? 0,
            'luas_bng_sppt' => $item['data_preview']['luas_bangunan'] ?? 0, 'no_sk' => $previewData['no_sk'], 'tgl_sk' => $previewData['tgl_sk'],
            'keterangan_pembatalan' => $previewData['keterangan'], 'berkas_path' => $berkasPath, 'status_proses' => $item['status'],
            'pesan_proses' => $item['message'], 'operator' => $user->name, 'nip_operator' => $user->nip,
        ]);
    }
    ...
    public function cetakFilteredPdf(Request $request) {
        ...
        if ($request->input('format') === 'excel') {
            ...
            foreach ($dataLaporan as $pembatalan) {
                $rowData = [
                    $pembatalan->formatted_nop, $pembatalan->thn_pajak_sppt, $pembatalan->nm_wp_sppt, $pembatalan->alamat_wp, $pembatalan->letak_op,
                    (float)($pembatalan->luas_bumi_sppt ?? 0), (float)($pembatalan->luas_bng_sppt ?? 0), (float)($pembatalan->pbb_yg_harus_dibayar_sppt ?? 0),
                    $pembatalan->no_sk, Carbon::parse($pembatalan->tgl_sk)->format('d-m-Y'), $pembatalan->created_at->format('d-m-Y H:i:s'), $pembatalan->operator,
                ];
