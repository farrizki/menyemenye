    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @forelse ($laporan as $data)
                <tr class="{{ $data->status_proses == 'Gagal' ? 'bg-red-50' : '' }}">
                    ...
                    <td class="px-3 py-4 whitespace-nowrap text-sm">{{ number_format($data->luas_bumi_sppt ?? 0, 0, ',', '.') }}</td>
                    <td class="px-3 py-4 whitespace-nowrap text-sm">{{ number_format($data->luas_bng_sppt ?? 0, 0, ',', '.') }}</td>
                    <td class="px-3 py-4 whitespace-nowrap text-sm">Rp {{ number_format($data->pbb_yg_harus_dibayar_sppt ?? 0, 0, ',', '.') }}</td>
                    <td class="px-3 py-4 whitespace-nowrap text-sm">{{ $data->no_sk }}</td>
                    <td class="px-3 py-4 whitespace-nowrap text-sm">{{ \Carbon\Carbon::parse($data->tgl_sk)->format('d-m-Y') }}</td>
                    ...
                    <td class="px-3 py-4 whitespace-nowrap text-sm text-center">
                        @if($data->berkas_path)
                            <a href="{{ Storage::url($data->berkas_path) }}" target="_blank" class="text-blue-600 hover:text-blue-900">
                    ...
                    <td class="px-3 py-4 whitespace-nowrap text-sm">
                        <div class="flex items-center space-x-2">
                            <a href="{{ route('pembatalan.cetak-single-pdf', $data->id) }}" target="_blank" class="text-blue-600 hover:text-blue-900" title="Cetak PDF">
                            ...
                            <a href="{{ route('pembatalan.edit', $data->id) }}" class="text-green-600 hover:text-green-900" title="Edit">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            <form action="{{ route('pembatalan.destroy', $data->id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus data ini? Tindakan ini akan mengembalikan data di Oracle ke semula.');">
                                @csrf
                                @method('DELETE')
...
<div class="mt-4 flex justify-between items-center">
    <div class="text-sm text-gray-600">
        Menampilkan {{ $laporan->firstItem() }} - {{ $laporan->lastItem() }} dari total {{ $laporan->total() }} data.
    </div>
    <div>
