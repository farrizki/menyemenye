{{-- resources/views/denda_administratif/partials/laporan_table.blade.php --}}

{{-- Informasi Jumlah Data --}}
<div class="mb-4 text-sm text-gray-600">
    Menampilkan {{ $laporanDenda->firstItem() }} - {{ $laporanDenda->lastItem() }} dari total {{ $laporanDenda->total() }} data.
</div>

{{-- Tabel Laporan yang Sudah Dirapikan --}}
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @forelse ($laporanDenda as $data)
                <tr>
                    ...
                    <td class="px-3 py-4 whitespace-nowrap text-sm">{{ $data->tgl_sk ? \Carbon\Carbon::parse($data->tgl_sk)->format('d-m-Y') : '-' }}</td>
                    <td class="px-3 py-4 whitespace-nowrap text-sm">{{ $data->tgl_jatuh_tempo_baru ? \Carbon\Carbon::parse($data->tgl_jatuh_tempo_baru)->format('d-m-Y') : '-' }}</td>
                    ...
                    <td class="px-3 py-4 whitespace-nowrap text-sm">
                        <div class="flex items-center justify-center space-x-3">
                            <a href="{{ route('denda_administratif.cetak-single-pdf', $data->id) }}" target="_blank" class="text-gray-500 hover:text-blue-600" title="Cetak PDF">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2z"></path></svg>
                            </a>
                            <a href="{{ route('denda_administratif.edit', $data->id) }}" class="text-gray-500 hover:text-green-600" title="Edit">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            <form action="{{ route('denda_administratif.destroy', $data->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                @csrf
                                @method('DELETE')
...
<div class="mt-4 flex justify-between items-center">
    <div></div>
    <div>
        {{ $laporanDenda->appends(request()->query())->links() }}
    </div>
</div>