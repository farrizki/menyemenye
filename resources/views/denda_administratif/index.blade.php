<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Laporan Penghapusan Denda Administratif') }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                                    ...
                                    <tbody>
                                        @foreach ($flashResults as $result)
                                            <tr class="{{ $result['status'] == 'Berhasil' ? 'bg-green-50' : ($result['status'] == 'Gagal' || $result['status'] == 'Error' ? 'bg-red-50' : 'bg-yellow-50') }}">
                                                <td>{{ $result['formatted_nop'] ?? $result['nop'] ?? '-' }}</td>
                                                <td>{{ $result['thn_pajak_sppt'] ?? '-' }}</td>
                                                <td>{{ $result['status'] }}</td>
                    ...
                    <div class="flex justify-end mb-4">
                        ...
                        <a href="{{ route('denda_administratif.filter-pdf') }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 active:bg-red-700 focus:outline-none focus:border-red-700 focus:ring ring-red-300 disabled:opacity-25 transition ease-in-out duration-150" target="_blank">
                    ...
                    <div class="mb-4 flex items-center space-x-2">
                        <x-text-input type="text" id="searchInput" placeholder="Cari NOP/Nama/Tahun/Alamat/No SK..." class="w-full" value="{{ request('search') }}" />
                        <a href="{{ route('denda_administratif.index') }}" id="resetButton" class="inline-flex items-center px-4 py-2 bg-gray-400 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-500 active:bg-gray-600 focus:outline-none focus:border-gray-600 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150 {{ request('search') ? '' : 'hidden' }}">Reset</a>
                    </div>

                    <div id="laporanTableContainer">
                        @include('denda_administratif.partials.laporan_table', ['laporanDenda' => $laporanDenda])
                    </div>
        ...
        <script>
            ...
            function fetchLaporan(searchQuery = '', page = 1) {
                ...
                searchTimeout = setTimeout(() => {
                    const url = new URL("{{ route('denda_administratif.index') }}");
